...
import { User, MapPin, Clock, Eye, EyeOff, X, ChevronDown } from 'lucide-react';
// import {X} from 'lucide-react';
import { useParams } from 'react-router-dom';
const UserProfileCard = ({ users, currentUser }) => {
  const { id } = useParams();
  ...
  // Sample current user (logged in user)
  const sampleCurrentUser = {
    id: 2,
    name: "John Doe",
    email: "john@example.com",
    skills_offered: "Node.js, Python, Machine Learning, Backend Development, Docker, AWS",
    skills_wanted: "React, Vue.js, Frontend Development"
  };

  // Use sample data for preview, replace with real user prop in production
  // const userData = user || sampleUser;
  const currentUserData = currentUser || sampleCurrentUser;
  ...
  const currentUserSkillsOffered = parseSkills(currentUserData.skills_offered);
  ...
  const handleSendRequest =async () => {
    const requestData = {
      fromUserId: sessionStorage.getItem('user_id'), // Assuming user ID is stored in sessionStorage
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert("Failed to send swap request: " + data.error);
      }
    } catch (err) {
      console.error("Error sending request:", err);
      alert("An error occurred while sending the swap request.");
    }
